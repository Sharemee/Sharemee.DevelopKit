...
namespace Sharemee.DevelopKit.Encrypt.RsaUtil;
...
public class RsaKeyConvert
{
    ...
    public static string PublicKeyXmlToPem(string publicKey)
    {
        ...
        //Modulus
        var modulus = root.Element("Modulus") ?? throw new Exception();
        //Exponent
        var exponent = root.Element("Exponent") ?? throw new Exception();

        var module = new BigInteger(1, Convert.FromBase64String(modulus.Value));
        var rsaKeyParameters = new RsaKeyParameters(false, module, new BigInteger(1, Convert.FromBase64String(exponent.Value)));
    ...
    public static string PrivateKeyPkcs1ToPkcs8(string privateKey)
    {
        ...
        AsymmetricCipherKeyPair kp = pr.ReadObject() as AsymmetricCipherKeyPair ?? throw new NullReferenceException("");
        StringWriter sw = new StringWriter();
        PemWriter pWrt = new PemWriter(sw);
